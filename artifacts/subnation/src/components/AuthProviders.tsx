...
function openTelegramLogin(botUsername: string): Promise<Record<string, string>> {
  return new Promise((resolve, reject) => {
    ...
    script.onerror = () => { clearTimeout(timeout); reject(new Error("تعذّر تحميل مكتبة Telegram")); };
...
function ProviderButton({
  ...
  const handle = async () => {
    ...
    try {
      ...
      if (provider.id === "google") {
        ...
        if (!clientId) { onError("لم يتم إعداد Google Client ID"); return; }
      ...
      if (provider.id === "telegram") {
        if (!provider.bot_username) { onError("لم يتم إعداد بوت Telegram"); return; }
  ...
  return (
    <button
      ...
      onClick={handle}
      disabled={loading}
      className={className ?? "w-full h-11 flex items-center justify-center gap-3 border border-border/60 rounded-xl bg-card hover:bg-muted/50 hover:border-border transition-all duration-150 active:scale-[0.97] font-medium text-sm disabled:opacity-60 press-spring"}
    >
    ...
    fetch("/api/auth/providers")
      ...
      .catch(() => {
        ...
        if (import.meta.env.VITE_GOOGLE_CLIENT_ID) {
          setProviders([{
            id: "google", label: "Google", color: "#4285F4", icon: "google",
            auth_type: "client_side", enabled: true, has_config: true,
