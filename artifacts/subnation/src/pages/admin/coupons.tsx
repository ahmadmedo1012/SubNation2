...
export default function AdminCouponsPage() {
  ...
  const handleCreate = async () => {
    if (!form.code.trim()) return toast({ title: "خطأ", description: "رمز الكوبون مطلوب", variant: "destructive" });
    const value = parseFloat(form.value);
    if (isNaN(value) || value <= 0) return toast({ title: "خطأ", description: "قيمة الخصم غير صالحة", variant: "destructive" });
