...
import {
  CheckCircle, XCircle, Bot, Key, Hash, Info, Bell, Shield,
  KeyRound, ToggleLeft, ToggleRight, Save, Loader2, Eye, EyeOff,
            ...
            {provider.fields.map(field => (
              <div key={field.key} className="space-y-1.5">
                <label className="text-xs font-bold text-muted-foreground flex items-center gap-1.5">
                  {field.label}
                  {field.isSecret && <span className="text-[10px] bg-yellow-400/10 text-yellow-400 border border-yellow-400/20 px-1.5 py-0.5 rounded font-bold">سري</span>}
                </label>
                <div className="relative">
                  <input
                    ...
                    placeholder={config[field.key] === "[SET]" ? "••••••••••• (مُعيَّن)" : (field.placeholder ?? "")}
                    dir="ltr"
...
export default function AdminSettingsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch("/api/admin/settings", { headers }).then(r => r.json()).catch(() => null),
      fetch("/api/admin/settings/auth", { headers }).then(r => r.json()).catch(() => ({ providers: [] })),
    ...
    <AdminLayout>
      <div className="space-y-6 max-w-3xl">
        <div>
          ...
          <p className="text-muted-foreground text-sm">إعدادات النظام والتكاملات وإدارة طرق المصادقة</p>
        </div>
        ...
        {activeTab === "auth" && (
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-3 px-5 py-3.5 bg-card border border-border rounded-xl">
              ...
              <span className={`text-xs font-black px-2.5 py-1 rounded-full border ${enabledCount > 0 ? "bg-primary/10 text-primary border-primary/20" : "bg-muted text-muted-foreground border-border"}`}>
              ...
              <div className="space-y-3">
                {providers.map(provider => (
                  <ProviderCard
                    ...
                    onUpdate={updated => setProviders(prev => prev.map(p => p.id === updated.id ? updated : p))}
                  />
                ))}
              </div>
            )}
            ...
            <div className="flex items-start gap-3 p-4 bg-muted/30 border border-border/50 rounded-xl">
              ...
              <div className="text-xs text-muted-foreground leading-relaxed space-y-1">
                ...
                <p>عند تفعيل مزود وإدخال بيانات الاعتماد، يظهر زر تسجيل الدخول به تلقائياً في صفحات الدخول والتسجيل.</p>
                <p>قيم <span className="font-mono bg-background/80 px-1 rounded border border-border/60">[SET]</span> تعني أن القيمة مُعيَّنة مسبقاً — أترك الحقل فارغاً لعدم تغييرها.</p>
              </div>
            </div>
          </div>
        )}
