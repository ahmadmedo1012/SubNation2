...
import {
  MessageSquare, Send, CheckCircle, Loader2, X,
  Clock, AlertCircle, ChevronLeft, User, Shield
} from "lucide-react";
import { useToast } from "@/hooks/use-toast";

const STATUS_CONFIG: Record<string, { label: string; color: string; dot: string }> = {
  open:        { label: "مفتوحة",        color: "text-blue-400 bg-blue-400/10 border-blue-400/20",   dot: "bg-blue-400" },
  in_progress: { label: "قيد المعالجة", color: "text-yellow-400 bg-yellow-400/10 border-yellow-400/20", dot: "bg-yellow-400" },
  closed:      { label: "مغلقة",         color: "text-muted-foreground bg-muted/30 border-border",   dot: "bg-muted-foreground" },
};
              ...
              <div className="flex items-start justify-between px-5 py-4 border-b border-border bg-muted/15">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className={`text-[11px] px-2 py-0.5 rounded-full border font-bold ${STATUS_CONFIG[selected.status]?.color}`}>
                      {STATUS_CONFIG[selected.status]?.label}
                    </span>
                    {selected.category && (
                      <span className="text-xs text-muted-foreground">{CATEGORIES[selected.category] ?? selected.category}</span>
                    )}
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4 min-h-0" style={{ maxHeight: "380px" }}>
                {selected.replies.length === 0 ? (
                  ...
                  selected.replies.map(r => {
                    ...
                    return (
                      <div key={r.id} className={`flex gap-2.5 ${isAdmin ? "flex-row-reverse" : "flex-row"}`}>
                        <div className={`w-7 h-7 rounded-full shrink-0 flex items-center justify-center text-xs font-bold mt-0.5 ${isAdmin ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}>
                        ...
                        <div className={`max-w-[78%] ${isAdmin ? "items-end" : "items-start"} flex flex-col gap-1`}>
                          <div className={`rounded-2xl px-4 py-2.5 ${isAdmin ? "bg-primary text-white rounded-tl-sm" : "bg-muted/60 border border-border/50 rounded-tr-sm"}`}>
                            <p className="text-sm leading-relaxed">{r.message}</p>
                          </div>
                          <div className={`flex items-center gap-1 text-[10px] text-muted-foreground ${isAdmin ? "flex-row-reverse" : ""}`}>
