...
const CATEGORY_ACCENT: Record<string, {
  ...
  gradient: string; imgBg: string; shadow: string; accentLine: string; hoverShadow: string;
}> = {
  streaming:    { bg: "bg-violet-500/10",  text: "text-violet-300",  border: "border-violet-500/20", gradient: "from-violet-950/95 via-violet-900/55 to-violet-800/25",  imgBg: "bg-violet-950/60", shadow: "shadow-violet-950/40",  accentLine: "bg-violet-500/55", hoverShadow: "hover:shadow-violet-900/25" },
  music:        { bg: "bg-emerald-500/10", text: "text-emerald-300", border: "border-emerald-500/20", gradient: "from-emerald-950/95 via-emerald-900/55 to-emerald-800/25", imgBg: "bg-emerald-950/60", shadow: "shadow-emerald-950/30", accentLine: "bg-emerald-500/55", hoverShadow: "hover:shadow-emerald-900/20" },
  gaming:       { bg: "bg-blue-500/10",    text: "text-blue-300",    border: "border-blue-500/20",   gradient: "from-blue-950/95 via-blue-900/55 to-blue-800/25",          imgBg: "bg-blue-950/60",    shadow: "shadow-blue-950/35",    accentLine: "bg-blue-500/55",   hoverShadow: "hover:shadow-blue-900/20"   },
  productivity: { bg: "bg-amber-500/10",   text: "text-amber-300",   border: "border-amber-500/20",  gradient: "from-amber-950/95 via-amber-900/55 to-amber-800/25",       imgBg: "bg-amber-950/60",   shadow: "shadow-amber-950/30",   accentLine: "bg-amber-500/55",  hoverShadow: "hover:shadow-amber-900/20"  },
};

const DEFAULT_ACCENT = {
  bg: "bg-primary/10", text: "text-primary", border: "border-primary/20",
  gradient: "from-primary/28 via-primary/12 to-transparent",
  imgBg: "bg-card", shadow: "shadow-primary/15", accentLine: "bg-primary/55", hoverShadow: "hover:shadow-primary/15",
};
  ...
  return (
    <Link href={`/product/${product.id}`}>
      ...
      `}>
        ...
        {/* Image area */}
        <div className={`relative h-[130px] sm:h-[148px] bg-gradient-to-b ${accent.gradient} flex items-center justify-center overflow-hidden`}>
          {/* Top accent line */}
          ...
          >
            <span className={`text-6xl font-black select-none opacity-25 group-hover:opacity-40 transition-all duration-300 drop-shadow-xl ${accent.text}`}>
              {product.name[0]}
            </span>
          </div>
        ...
        <div className="p-3.5 pt-3">
          ...
          {/* Description */}
          {product.description && (
            <p className="text-muted-foreground/65 text-[11px] line-clamp-2 leading-relaxed mb-2.5">{product.description}</p>
          )}
