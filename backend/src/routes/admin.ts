...
router.get("/orders", requireAdmin, async (req, res) => {
  ...
  const limit = Math.min(Math.max(Number.parseInt(queryString(req, "limit", "100"), 10) || 100, 1), 200);
...
router.get("/referrals", requireAdmin, async (req, res) => {
  ...
  const rows = rowsFromResult<any>(await db.execute(sql`
    SELECT
      re.id,
      re.status,
      re.created_at,
  ...
  const topReferrers = rowsFromResult<any>(await db.execute(sql`
    SELECT
      u.phone,
      u.id,
  ...
  const [statsRow = {}] = rowsFromResult<any>(await db.execute(sql`
    SELECT
      COUNT(*) AS total,
