...
| C4  | **Wallet balance mutated directly on `users` row**    | `routes/orders.ts:194`, `services/topup.service.ts`. Ledger is written _after_ the transaction commits (`insertLedgerEntry` outside `tx`).                                                             | Ledger can desync from `users.walletBalance` if process crashes between commit and ledger insert. Audit trail untrustworthy.                |
